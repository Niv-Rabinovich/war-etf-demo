...
st.set_page_config(page_title="WAR (6F) ETF 3X | קסם קרנות נאמנות", page_icon="🛡️", layout="wide")
...
# ── Definitions ─────────────────────────────────────────────────────────────────
SECTORS = {
    "סקטור 1: תעשיות ביטחוניות ונשק 🛡️": {
        "stocks": ["LMT", "RTX", "NOC", "GD", "HII", "LHX"],
        "color": "#3b82f6", "he": "תעשיות ביטחוניות ונשק",
...
FUND_STRUCTURE = {
    "שם הקרן": "WAR (6F) ETF 3X",
    "מנהל הקרן (מנפיק)": "קסם קרנות נאמנות בע\"מ (2003)",
    "נאמן": "מזרחי טפחות חברה לנאמנויות בע\"מ",
    "סוג קרן": "קרן פטורה",
    "דמי ניהול": "2.00% לשנה — מנוכה יומית",
    "מינוף": "3x — ניהול ע\"י מנוע AI",
    "בורסה": "הבורסה לני\"ע בתל אביב (TASE)",
    "מספר מניות": "30 מניות (6 בכל סקטור)",
    "משקל סקטור": "20% לכל אחד מ-5 הסקטורים",
    "יחידות יצירה": "500,000 (משתתפים מורשים · in-kind/מזומן)",
...
st.markdown(f"""
<div class="hero">
    <div class="hero-tag" style="margin:0 0 8px;color:{ACCENT}">◆ קסם קרנות נאמנות בע״מ</div>
    <div class="hero-title">🛡️ WAR (6F) ETF 3X</div>
    <div class="hero-sub">תעודת סל ממונפת ×3 — חשיפה ממוקדת לכלכלת מלחמה</div>
    <div class="hero-tag">הנפקה ראשונה לציבור · ניהול ע״י מנוע AI</div>
</div>
""", unsafe_allow_html=True)
...
tab_story, tab_perf, tab_sim, tab_biz, tab_prospectus = st.tabs([
    "🎯 ההזדמנות",
    "📈 ביצועים וסיכון",
    "💰 סימולציית השקעה",
    "💼 מודל עסקי ומבנה",
...
ITA/XAR/PPA הן ×1 בלבד ולא כוללות סייבר, אנרגיה וספנות.
    ...
    with st.expander("⚙️ מנוע ניהול המשקולות — אכיפת תקרת 20% למניה"):
        ...
        st.caption(f"מנוע ה-AI מקצה את המשקולות ואוכף **תקרה קשיחה של 20% למניה** עם דילול אוטומטי בחריגה. "
                   f"המשקל הגבוה ביותר כרגע: **{mx:.1f}%** — ✅ כל המניות מתחת לתקרה "
                   f"(בדמו: סל ייצוגי במשקל שווה).")

    with st.expander("📋 מתודולוגיה וגילוי נאות — חשוב לקרוא"):
        st.markdown(f"""
**ניהול ע"י מנוע AI (לפי התשקיף):** הרכב הקרן אינו קבוע ידנית — מנוע בינה מלאכותית של מנהל הקרן בוחר ומעדכן
**דינמית** 6 מניות בכל סקטור (סה"כ 30), לפי מתאם גבוה לאירועי הסלמה ומתח גיאופוליטי. משקל 20% לכל סקטור,
עם תקרת משקל קשיחה למניה בודדת ודילול אוטומטי.

**ℹ️ לגבי הדמו:** להמחשה, האתר מציג סל **ייצוגי קבוע** של 30 המניות ומריץ עליו Backtesting. במוצר עצמו המנוע
מעדכן את ההרכב דינמית. ביצועי העבר **אילוסטרטיביים ואינם תחזית לעתיד**.
...
# ═══════════════════════════════════════════════════════════════════════════════
# TAB 5 — תשקיף (הנפקה ראשונה לציבור)
# ═══════════════════════════════════════════════════════════════════════════════
with tab_prospectus:
    st.markdown("## 📄 תשקיף — הנפקה ראשונה לציבור")
    st.markdown("### תעודת סל: **WAR (6F) ETF 3X**")
    st.divider()

    st.markdown("""
#### 1. תמצית התשקיף ופרטי המנפיק
- **1.1 שם המנפיק ומבנה משפטי:** קסם קרנות נאמנות בע״מ (התאגדה ב-2003).
- **1.2 תיאור כללי:** נייר הערך המוצע הוא תעודת סל הרשומה למסחר בבורסה לני"ע. הקרן מציעה חשיפה ישירה לחמישה סקטורים.
- **1.3 מטרת הגיוס:** התמורה מהנפקת היחידות תשמש לרכישת נכסי המעקב של המדד, בניכוי עמלות ודמי ניהול.

#### 2. הרכב נכסים מיועד ומשקולות
מניות הקרן והרכבן **אינם קבועים מראש ידנית**. מנוע **בינה מלאכותית (AI)** של מנהל הקרן קובע ומעדכן **באופן דינמי** את המניות בכל אחד מחמשת הסקטורים, כאשר משקל כל סקטור = 20% מהקרן, בכפוף ל:
- **מבנה קשיח:** בדיוק **6 מניות מכל סקטור** (סה"כ **30 מניות** בקרן).
- **מתאם גבוה לאירועים גיאופוליטיים:** מודל ה-AI מזהה ברציפות מניות בעלות רגישות גבוהה לאירועי הסלמה, מתח צבאי ושינויים גיאופוליטיים.

**הסקטורים (20% לכל אחד):**
1. **תעשיות ביטחוניות ונשק** — תעשייה כבדה ויצרניות נשק בעלות חוזים ממשלתיים קשיחים.
2. **סייבר הגנתי** — אבטחה מתקדמת לענן, תשתיות קריטיות ורשתות דיגיטליות.
3. **אנרגיה מסורתית** — הפקה, קידוח וזיקוק נפט וגז, רגישות לשיבושי אספקה.
4. **ספנות ותובלה ימית** — ציי מיכליות ואוניות משא הנהנות מזינוק במחירי השילוח בעת משברים.
5. **תקשורת לוויינית ומודיעין AI** — לוויינים, תקשורת מוצפנת ומערכות ניתוח מודיעין בזמן אמת.

#### 3. מבנה עלויות, דמי ניהול ועמלות
- **דמי ניהול שנתיים:** **2.00%** משווי הנכסים המנוהל (AUM), מנוכים יומית משווי היחידה.
- **עמלת קנייה/מכירה (יצירה ופדיון):** בהתאם למוסד הפיננסי בו נקנתה התעודה.
- **טעות עקיבה צפויה (Expected Tracking Error):** אם הקרן לא עמדה במינוף ×3 — יוחזרו "דמי ניהול שליליים" ללקוח; אם עברה את התשואה הממונפת ×3 — תיקח דמי ניהול חיוביים לעצמה.

#### 5. גורמי סיכון
- **5.1 סיכון מטבע:** שינויים בשער החליפין ₪/$ עלולים להקטין את תשואת הקרן.
- **5.2 סיכון נזילות:** ייתכן שלא ניתן יהיה לקנות/למכור יחידות במחיר הוגן בהיעדר עושי שוק פעילים.
- **5.3 סיכון עקיבה (Tracking Error):** ביצועי הקרן עלולים שלא להתאים במדויק למדד הייחוס בשל עלויות, עמלות או פערי תזמון.

#### 6. מנגנון יצירה ופדיון (Creation & Redemption)
יצירה ופדיון יתבצעו **רק באמצעות משתתפים מורשים**, בחבילות מינימליות של **500,000 יחידות** (Creation Units), תמורת סל המניות (In-Kind) או מזומן, לפי שיקול דעת מנהל הקרן.

#### 7. הצהרת מנהלים
המנהלים מצהירים כי למיטב ידיעתם התשקיף משקף נאמנה את מדיניות ההשקעה, מבנה העלויות וגורמי הסיכון של תעודת הסל המוצעת.
""")
    st.caption("📌 המסמך מובא לצרכי לימוד והדגמה — אינו תשקיף משפטי מחייב.")

# ── Footer ──────────────────────────────────────────────────────────────────────
st.divider()
st.markdown(f"""<div style="text-align:center;color:{MUTED};font-size:12px;padding:8px 0">
    WAR (6F) ETF 3X — קסם קרנות נאמנות בע״מ &nbsp;|&nbsp; נתונים: Yahoo Finance · TASE/גלובס<br>
